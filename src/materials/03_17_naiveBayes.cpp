...
void GNB::train(vector<vector<double>> data, vector<string> labels)
{
   ...
   for (int i=0; i<labels.size(); i++){
      if (labels[i] == "left"){
         left_means += Eigen::ArrayXd::Map(data[i].data(), data[i].size()); //conversion of data[i] to ArrayXd
         left_size += 1;
      } 
      else if (labels[i] == "keep") {
         keep_means += Eigen::ArrayXd::Map(data[i].data(), data[i].size());
         keep_size += 1;
      } else if (labels[i] == "right") {
         right_means += Eigen::ArrayXd::Map(data[i].data(), data[i].size());
         right_size += 1;
      }
   }
